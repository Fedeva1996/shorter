...
- [x] **T2.5.2** — Implementar la validación anti-bucle en `core/urlValidator.ts` (Zod `refine`).
- [x] **T2.5.3** — Actualizar test de integración de POST `/api/v1/urls` comprobando la idempotencia (EC-07).
- [x] **T2.5.4** — Implementar lógica de idempotencia en `url.service.ts` y ajustar códigos 200/201 en `url.controller.ts`.
- [x] **T2.5.5** — Escribir test de integración para Rate Limiting (EC-06).
- [x] **T2.5.6** — Instalar dependencias `express-rate-limit` y `helmet`. Configurar middlewares en `app.ts`.

---

## Fase 3: Frontend (React)

- [x] **T3.1** — Definir la interfaz TypeScript compartida para la respuesta de creación de URL.
- [x] **T3.2** — Escribir test (RTL) para `ShortenerForm.tsx`: el formulario no debe permitir enviar el submit si el input está vacío o es una URL obviamente malformada.
...
- [x] **T4.2** — Abrir sesión del Adversario pasándole los archivos críticos (`url.service.ts`, `url.controller.ts`, esquemas de Prisma y validaciones) para buscar brechas (focus en incrementos atómicos y bloqueos por unicidad).
- [x] **T4.3** — Corregir hallazgos del Adversario si existieran, iterando hasta el `"VERIFICADO ✓"`.
- [x] **T4.4** — Marcar Criterios de Éxito completados en el `SPEC.md`.